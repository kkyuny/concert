...
- 유효시간(regiDate + vaildTime) 동안에 해당 row가 존재하는 한 해당 콘서트의 좌석에 동일한 콘서트와 자석의 insert 불가
- 유효시간 내 결제 완료 시 status -> 예약완료로 변경 -> 새로운 예약정보 insert 불가
- 유효시간 내 결제 미완료시 해당 row 삭제처리(예약 history table에 신청 정보는 남아 있음) -> 새로운 예약정보 insert가 가능하게 만듬
